...
export class GameEngine {
	static readonly DRUNKINESS_LOSS: number = 0.05;
	...
	getSpawnableTile(): { x: number; y: number } {
		return this.spawnableTiles[
			Math.floor(Math.random() * this.spawnableTiles.length)
		];
	}

	getPickSpawnableTile(): { x: number; y: number } {
		return this.pickupSpawnableTiles[
			Math.floor(Math.random() * this.pickupSpawnableTiles.length)
		];
	}
}
